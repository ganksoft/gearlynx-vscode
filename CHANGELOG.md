...
- A lost debug-monitor connection now also shows a toast (previously log-only),
  since it ends the debug session.
- The Screen Viewer now shows framebuffer stream errors in its status line, and
  blacks out instead of keeping the last frame when the debug session ends.
